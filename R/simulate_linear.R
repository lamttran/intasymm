...
simulate_linear = function(K = 3, p = 5, n = c(50, rep(200, K - 1)), beta0 = rnorm(p, 0, 1), 
                           K0 = 1, sigma.beta = 1, mu.x = 0, sigma.x = 1, numfrac = 0, 
                           sigma.y = 0.5, correlated = F, equalcor = F, trace = F) {
  ...
  for (i in 1:K) {#looping over datasets
    ...
    if(correlated == F){
      ...
      x = matrix(rnorm(n[i] * length(beta[[i]]), mean = mu.x[i], 
                       sd = sigma.x[i]), ncol=length(beta[[i]]))
